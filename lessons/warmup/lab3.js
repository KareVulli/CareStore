...
const isNumberValid = (inputString, {precision = 9, scale = 0}) => {
  ...
  let decimalsCount = (temp[1] ? temp[1].length : 0);
  if (inputString.indexOf('.') > -1 && decimalsCount == 0) {
    return false;
  }
  if (decimalsCount > scale) {
    return false;
  }
  let numbersCount = temp[0].length + decimalsCount;
  if (numbersCount > precision) {
    return false;
  }
  return true;
};
