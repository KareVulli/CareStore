 ...
 * Task 1
 */
const simpleArraySum = xs => xs.reduce((acc, e) => acc += e, 0)
...
const generateRandomItemList = n => {
    ...
    while (set.size < n) {
        set.add(getRandomIntInclusive(0, 10000))
    }
    let array = []
    set.forEach(el => {
        array.push({
 ...
 * Task 4
 */
const findMostExpensiveItem = items => items.reduce((acc, item) => acc.cost > item.cost ? acc : item);

/**
 * Task 5
 */
const findCheapestItem = items => items.reduce((acc, item) => acc.cost < item.cost ? acc : item);
 ...
 * 
 */
const searchItems = (items, re) => items.reduce((acc, item) => {
